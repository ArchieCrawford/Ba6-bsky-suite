...
export async function POST(request: Request, context: { params: Promise<{ feedId: string }> }) {
  try {
    ...
    if (body.gateId) {
      const { error } = await supa.from("feed_gates").update(payload).eq("id", body.gateId).eq("feed_id", feedId);
      if (error) throw error;
...
export async function DELETE(request: Request, context: { params: Promise<{ feedId: string }> }) {
  try {
    ...
    const { error } = await supa.from("feed_gates").delete().eq("id", body.gateId).eq("feed_id", feedId);
    if (error) throw error;
